...
class Village(object):
    def __init__(self, web_driver, name, homepage):
        ...
        self.center_page = 'https://ts3.travian.it/dorf2.php'
        self.resources_available = None
        self.resources_prod = None
        #self.crops = None
        self.crops = VF.crops_get(self.driver)
        self.buildings = VF.building_get(self.driver)
        self.troops_av = None
    ...
    def __repr__(self):
        ...
        return "village -\n name: {0},\n link: {1},\n upgrade status: {2},\n res prod: {3},\n res av: {4},\n troops av: {5}\n buildings: {6}\n crops: {7}".format(
            self.name, self.homepage, self.construction_status, self._resources_prod, self._resources_available, self._troops_av, self.buildings, self.crops)
    def __str__(self):
        #return "village - name: {0}, link: {1}, upgrade status: {2}, res prod: {3}, res av: {4}, troops av: {5}, crops: {6}".format(
        #    self.name, self.homepage, self.upgrade_status, self._resources_prod, self._resources_available, self._troops_av, self._crops)
        return "village -\n name: {0},\n link: {1},\n upgrade status: {2},\n res prod: {3},\n res av: {4},\n troops av: {5}\n buildings: {6}\n crops: {7}".format(
